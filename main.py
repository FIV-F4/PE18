...
def send_welcome(message):
    bot.reply_to(message,
                 text="Привет! Я бот для создания озвучки! Выбери голос, который будет использоваться при создании озвучки:",
...
def voice_selected(message):
    ...
    bot.reply_to(message, text=f"Вы выбрали голос: {message.text}. Теперь введите текст для озвучки:")
...
def generate_voice(message):
    ...
    if user_id in selected_voice:
        ...
        bot.reply_to(message, text="Сначала выберите голос командой /start")
